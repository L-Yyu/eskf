...
bool ErrorStateKalmanFilter::Init(const GPSData &curr_gps_data, const IMUData &curr_imu_data) {
    ...
    Eigen::Quaterniond Q_init = Eigen::AngleAxisd(config_parameters_.init_yaw_ * kDegree2Radian, Eigen::Vector3d::UnitZ()) *
                                Eigen::AngleAxisd(config_parameters_.init_pitch_ * kDegree2Radian, Eigen::Vector3d::UnitY()) *
                                Eigen::AngleAxisd(config_parameters_.init_roll_ * kDegree2Radian, Eigen::Vector3d::UnitX());
...
void ErrorStateKalmanFilter::UpdateOdomEstimation(const Eigen::Vector3d &w_in) {
    ...
    Eigen::Vector3d delta_rotation = ComputeDeltaRotation(last_imu_data, curr_imu_data);    //计算姿态角旋转角度
    
    // 计算地球自转引起的旋转矩阵，如果use_earth_model: false，则为单位矩阵
    ...
    Eigen::Matrix3d curr_R; // curr旋转矩阵
    Eigen::Matrix3d last_R; // last旋转矩阵
    ...
    // std::cout<<"pose:   "<<pose_.block<3, 1>(0, 3)<<std::endl;
...
void ErrorStateKalmanFilter::ComputeOrientation(const Eigen::Vector3d &angular_delta,
    ...
    Eigen::AngleAxisd angle_axisd(angular_delta.norm(), angular_delta.normalized());//根据姿态角变化计算旋转矩阵
